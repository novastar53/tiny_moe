...
class Tiny_MoE(nnx.Module):
    def __init__(self, config: Config, rngs: nnx.Rngs):
        ...
        self.pos = nnx.Embed(config.block_size, config.n_embed, rngs=rngs)
        self.layers = []
    ...
    @staticmethod
    def from_checkpoint(
        fpath: str, rngs: nnx.Rngs, config: Optional[Config] = None, sharding: Optional[jax.sharding.NamedSharding] = None
    ):
        ...
        for i in range(len(model.layers)):
            if hasattr(model.layers[i], "moe"):
                ...
                model.layers[i].moe.gate_noise_rngstream = rngs.gate_noise # TODO: Temporary fix for backward compatibility with jax 0.5.2
        return model
